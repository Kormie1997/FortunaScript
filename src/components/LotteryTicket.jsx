...
function LotteryTicket({ game, onBack, user, onAddToCart }) {
  ...
  const quickPick = (panelId) => {
    ...
    //Eurojackpot: bónusz is mezőnként
  ...
  //Validáció vásárlás/kosár előtt
  ...
  //Payload összeállítása a kosárba helyezéshez vagy vásárláshoz
  ...
  const renderNumberGrid = (panel) => {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${config.gridCols}, minmax(0, 1fr))`, gap: '3px', marginTop: '8px' }}>
        {nums.map(num => {
          const isSelected = panel.numbers.includes(num);
          const canSelect  = panel.numbers.length < config.maxNumbers || isSelected;
          return (
            <button key={num} onClick={() => toggleNumber(panel.id, num)}
              disabled={panel.isLocked || (!canSelect && !isSelected)}
              className={`aspect-square rounded font-medium text-xs transition-all border flex items-center justify-center
                ${isSelected ? 'bg-red-600 text-white border-red-600 font-bold'
                  : panel.isLocked ? 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed'
                  : canSelect ? 'bg-white text-gray-700 border-gray-300 hover:bg-amber-50 hover:border-amber-400'
    ...
    <div className="space-y-6">
      ...
      <div className="ticket-wrapper">
        <div className="ticket-sidebar">
          ...
          {jokerNumbers.map((num, idx) => (
            <div key={idx} className="mb-2">
              <div className="joker-row">
                <input type="text" maxLength={6} value={num}
                  onChange={e => handleJokerChange(idx, e.target.value)} placeholder="------"
              ...
              {jokerErrors[idx] && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={12} /> {jokerErrors[idx]}</p>}
            </div>
          ))}
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">Gépi játékok (0-{MAX_EXTRA})</label>
          ...
          <div className="price-box">
            ...
            <div>ÁR ÖSSZESEN: <span className={insufficientFunds ? 'text-red-500' : ''}>{totalPrice.toLocaleString()} Ft</span></div>
          </div>

          {insufficientFunds && (
            <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-red-600 text-xs flex items-center gap-1"><AlertCircle size={12} /> Hiányzik: {(totalPrice - (user?.balance || 0)).toLocaleString()} Ft</p>
            </div>
          )}

          <button onClick={handleAddToCart} disabled={totalPrice === 0 || insufficientFunds}
          ...
          <button onClick={handlePlay} disabled={isSubmitting || totalPrice === 0 || insufficientFunds}
              ...
              <div className="ticket-header">
                <div className="flex items-center gap-2">
                  ...
                  {panel.numbers.length > 0 && (
                    <span className={`text-xs px-2 py-0.5 rounded-full ${isPanelFilled(panel) ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
